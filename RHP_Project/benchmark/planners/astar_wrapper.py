...
def _heuristic(a: Tuple[int, int], b: Tuple[int, int]) -> float:
    ...
    dr = abs(a[0] - b[0])
    dc = abs(a[1] - b[1])
    return float(max(dr, dc) + (np.sqrt(2.0) - 1.0) * min(dr, dc))
    ...
    while open_set and n_expanded < max_expand:
        ...
        for dr, dc in _NBR8:
            ...
            if (nr, nc) not in g_score or tentative < g_score[(nr, nc)]:
                ...
                f_score = tentative + _heuristic((nr, nc), goal)
...
class AStarPlanner(BasePlanner):
    """A* planner on occupancy grid. Baseline for optimal path length."""

    def __init__(self, max_expand: int = 250000, diagonal: bool = True):
    ...
    def plan(self, scene: Scene) -> PlanningResult:
        ...
        t0 = time.time()
        path_np, n_expanded, timeout = _astar_search(
        ...
        path_pixel = path_np
        diffs = path_pixel[1:] - path_pixel[:-1]
        ...
        return PlanningResult(
            ...
            inference_time=elapsed,
            n_expanded=n_expanded,
            metadata={"astar_expanded": n_expanded,
                      "astar_timeout": timeout},
        )

    def config_snapshot(self) -> dict:
        return {"planner": "astar", "max_expand": self.max_expand,
                "diagonal": self.diagonal, "heuristic": "octile"}
